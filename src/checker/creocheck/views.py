...
def serialize_task(checktask):
    d = [
        checktask.pk, checktask.assignment.name, checktask.file.display_name, checktask.progress,
        checktask.get_status(), checktask.passed, None
    ]
    return d
    return json.dumps(d)
...
def checks_admin(request):
    ...
    try:
        selected_assignment = Assignment.objects.get(pk=request.session.get("active_assignment"))
        tasks = CheckTask.objects.filter(assignment=selected_assignment).order_by("-passed", "-created")
    except:
...
def admin_excel_export(request):
    ...
    header = ["Check ID", "User ID", "Timestamp", "Assignment", "Collection", "Passed"]

    wb = Workbook()
    ws = wb.active
    ws.append(header)
    for t in tasks:
        ws.append([t.pk, str(t.user), str(t.created), t.assignment.name, t.assignment.collection.name, str(t.passed)])
    out = io.BytesIO()
    wb.save(out)

    date_str = (datetime.datetime.now().strftime("CADVER_%y%m%d_%H%M%S"))
    response = HttpResponse(save_virtual_workbook(wb), content_type='application/vnd.ms-excel')
    response['Content-Disposition'] = 'attachment; filename={}.xlsx'.format(date_str)
    return response
...
def update_tasks(request):
    checktasks = CheckTask.objects.filter(user=get_user(request)).order_by("-pk")[:6]
...
def activate_user_collection(request):
    ...
    if len(user_id) < 5 or len(user_id) > 7:
        return HttpResponseBadRequest("Invalid link")

    assignment_collection_name = (request.GET.get("ac_name"))

    request.session['active_user_pk'] = CheckUser.objects.get_or_create(user_id=user_id)[0].pk
    try:
        request.session['active_collection_pk'] = AssignmentCollection.objects.get(name=assignment_collection_name).pk
...
def index(request):
    ...
    context = {}
    try:
        collection = AssignmentCollection.objects.get(pk=request.session.get('active_collection_pk'))
    except:
        return HttpResponse("USE LINK TO ACTIVATE SESSION.")

    context.update({'assignments': Assignment.objects.filter(collection=collection)})
...
def receive_file(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            assignment = Assignment.objects.get(name=request.POST.get("assignment_name"))

            new_file = UploadFile(file=f, display_name=display_name)
            new_file.save()

            a = CheckTask.objects.create(assignment=assignment, file=new_file, user=get_user(request))

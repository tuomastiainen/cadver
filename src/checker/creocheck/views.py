...
def checks_admin(request):
    ...
    try:
        ...
        tasks = CheckTask.objects.filter(assignment=selected_assignment).order_by("-created", "passed")
    except:
        tasks = CheckTask.objects.all().order_by("-created", "passed")
    context = {}
...
def index(request):
    ...
    context = {}
    context.update({'assignments': Assignment.objects.all()})
